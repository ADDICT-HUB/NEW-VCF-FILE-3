...
/* ============================
  Admin Login + Exports
============================ */
const ADMIN_PASSWORD_HASH = "c2e2b6d7f5ca32b6c8c2e6c9b9a8f2c6f36f9f3ae8a5a4a9f0ef1bb1e6f8f4d2";
...
function onAdminSuccess() {
  ...
  btnCsv.style.cssText = "padding:8px 10px;border-radius:8px;border:1px solid #333;background:#111;color:#fff";
  btnCsv.onclick = exportCSV;

  const btnVcf = document.createElement("button");
  btnVcf.innerText = "Download VCF";
  btnVcf.style.cssText = "padding:8px 10px;border-radius:8px;border:1px solid #333;background:#111;color:#fff";
  btnVcf.onclick = exportVCF;

  const btnPdf = document.createElement("button");
  btnPdf.innerText = "Download PDF";
  btnPdf.style.cssText = "padding:8px 10px;border-radius:8px;border:1px solid #333;background:#111;color:#fff";
  btnPdf.onclick = exportPDF;

  const btnLogout = document.createElement("button");
  btnLogout.innerText = "Logout";
  btnLogout.style.cssText = "padding:8px 10px;border-radius:8px;border:1px solid #333;background:#440;color:#fff";
  btnLogout.onclick = () => { adminLoggedIn = false; wrap.remove(); };

  wrap.append(btnCsv, btnVcf, btnPdf, btnLogout);
...
async function exportCSV() {
  ...
  if (error) return alert("CSV export failed.");
...
async function exportVCF() {
  if (!adminLoggedIn) return alert("Sign in first (Ctrl+Shift+A).");
  const { data, error } = await supabase.from(TABLE).select("*").order("id", { ascending: true });
  if (error) return alert("VCF export failed.");
  if (!data || data.length === 0) return alert("No data to export.");
...
async function exportPDF() {
  ...
  const { data, error } = await supabase.from(TABLE).select("*").order("id", { ascending: true });
  if (error) return alert("PDF export failed.");
  if (!data || data.length === 0) return alert("No data to export.");

  const { jsPDF } = window.jspdf;
  const doc = new jsPDF({ unit:"pt", format:"a4" });
  doc.setFontSize(12); doc.text("VCF Entries", 40, 40);
  let y = 60; const lineHeight = 14;
  data.forEach((r, idx) => {
    const rowText = `${idx+1}. ${(r.name||"").replace(/[<>]/g,"")} — ${(r.phone||"").replace(/[<>]/g,"")} — ${r.created_at ? new Date(r.created_at).toLocaleString() : ""}`;
...
function loadScript(src) {
  return new Promise((resolve,reject)=>{
    ...
    s.onload=()=>setTimeout(resolve,50); s.onerror=reject;
